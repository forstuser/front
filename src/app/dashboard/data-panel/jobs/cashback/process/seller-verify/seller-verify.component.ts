...
export class SellerVerifyComponent implements OnInit {
  ...
  getUserSellers() {
    this.__userService.getUserSeller(this.cashbackId).subscribe(res => {
  ...
  verifySeller(res: NgForm) {
    ...
    if (!res.value.document_number) {
      ...
      let request_data = {
        seller_id: seller_id,
        document_number: res.value.document_number,
        document_date: date,
        gstin: res.value.gstin
      };
      if (request_data.seller_id == undefined) {
        delete request_data["seller_id"];
      }
      this.__userService.verifySeller(request_data, this.cashbackId).subscribe(
        res => {
  ...
  updateSeller(res: NgForm) {
    console.log(res.value);
    this.__userService.updateSeller(res.value, this.sellerId).subscribe(
      res => {
      ...
      err => {
        if (this.jobDetails) {
          console.log("error", err);
          this.__ngxNotificationService.sendMessage(
            "Some Error Occured Seller not updated, Contact Admin",
            "dark",
            "bottom-right"
          );
          this.showSellerForm = false;
          ...
          this.__ngxNotificationService.sendMessage(
            "Some Error Occured, Contact Admin",
            "dark",
            "bottom-right"
          );
          this.__router.navigateByUrl("dashboard/newSeller");
  ...
  sellerMismatch() {
    this.__userService
      .discardJOB([{ id: this.cashbackId, reason_id: 1 }])
      .subscribe(
        res => {
          // console.log("res", res);
          this.__ngxNotificationService.sendMessage(
            "Job discarded due to seller mismatch moved to new job",
            "dark",
            "bottom-right"
          );
  ...
  public getCities(stateID: number) {
    this.__userService.getCities(stateID).subscribe(
      res => {
