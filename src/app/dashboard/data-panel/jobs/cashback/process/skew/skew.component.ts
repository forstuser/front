...
export class SkewComponent implements OnInit {
  ...
  ngOnChanges() {
    ...
    if (this.jobDetails) {
      ...
      this.skus.map((item, index) => {
        let res = item.sku_measurement_details.filter(
          sku => sku.id == item.sku_measurement_id
        );
        // console.log('res', res)
        if (res) {
          this.skuData.push({
            id: item.id,
            sku_id: item.sku_id,
            name: item.sku_details.title,
            measurement_value: res[0].measurement_value,
            measurement_type: res[0].measurement_type.acronym,
            pack_numbers: res[0].pack_numbers,
            cashback_percent: res[0].cashback_percent,
            quantity: item.quantity,
            mrp: res[0].mrp,
            sku_measurement_id: item.sku_measurement_id,
  ...
  cashback(res: NgForm) {
    ...
    for (let val = 0; val < sellingPrice.length; val++) {
      let calculate =
        (sellingPrice[val] * this.skuData[val].cashback_percent) / 100;
    ...
    for (let i = 0; i < this.skuIds.length; i++) {
      this.skuItems.push({
        id: this.skuIds[i],
        sku_id: this.skuData[i].sku_id,
        quantity: this.skuData[i].quantity,
        sku_measurement_id: this.skuData[i].sku_measurement_id,
        added_date: this.skuData[i].added_date,
        selling_price: sellingPrice[i],
        available_cashback: availableCashback[i]
      });
    }
